...
app = dash.Dash(__name__, include_assets_files=True)
    ...
    [
        ...
        html.Div(
            [
                ...
                dcc.Input(id="txt_search", type="text", placeholder="Search..."),
            ]
        ),
...
app.layout = html.Div(
    [
        dcc.Tabs(
            [
                dcc.Tab(label="News summary", children=[article_summary_layout]),
                dcc.Tab(label="Statistics & analysis", children=[statistics_layout]),
            ]
        )
    ]
)
...
def get_articles(n_clicks, search_value):
    ...
    with open("example2.json") as f:
        ...
        article_elements = [
            ArticleElement(data).to_html() for data in f_data["articles"]
        ]
